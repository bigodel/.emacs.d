...
mantained up to date via =el-get=. The following code is to load the Proof
General in the supported proof assistant files, with its respective modes.

This is the =el-get= config. Here we set the =el-get= path to be a
