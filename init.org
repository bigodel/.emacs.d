...
** TODO Add Magit-todos and hl-todo
** TODO Add flycheck
** TODO Add org config
...
#+BEGIN_SRC emacs-lisp :tangle config/config-bindings.el
...
I like using =helm= to pretty much anything I can, so I set most of
the common variables as =helm= commands. I also find it useful to
remap the =helm-command-prefix= to =C-c h=, because it is easy to
mistype =C-x C-c= with the default prefix.
...
=config/config-bindings.el= file, so they have their own.
...
#+BEGIN_SRC emacs-lisp :tangle config/config-misc.el
...
=paradox= is a better packages list. =popwin= is used to better manage pop-up
windows. =restart-emacs= makes it easier to restart your config and load new
code snippets.
...
#+BEGIN_SRC emacs-lisp :tangle config/config-eshell.el
...
#+BEGIN_SRC emacs-lisp :tangle config/config-vcs.el
...
#+BEGIN_SRC emacs-lisp :tangle config/config-flycheck.el
...
#+BEGIN_SRC emacs-lisp :tangle config/config-auxiliary-modes.el
...
#+BEGIN_SRC emacs-lisp :tangle config/config-company.el
...
#+BEGIN_SRC emacs-lisp :tangle config/config-yasnippet.el
  (/boot/delayed-init
...
#+BEGIN_SRC emacs-lisp :tangle config/config-projectile.el
